...
# reads the words text file and returns it as a LIST on the class/instance
# list is initiated when a Boggle instance is created because it is called in the init func
...
#makes the board out of an empty array, basically looping 5 times and creating a list for each Y coordinate out of 5 random letters generated by string.ascii uppercase
# and then appending that to the empty list    
    def make_board(self):
...
# checking if a word is valid on the board by checking against self.words using the find function defined later
# find sees if the combination of letters or the word to check is on the board and we compare to self.words to see if the letter combo is indeed a word0
    ...
    # valid2=game.check_valid_word(board2,"push")
    #valid2>>> 'ok'


# this is how we are actually checking the board
